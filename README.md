
# Nokuthula Ngcongo — IT Support Portfolio

## 🌐 Live Portfolio
My portfolio highlights my journey as an IT Support Specialist, showcasing:
- Hands-on troubleshooting labs (e.g., No Internet Access, Display Issues)
- Networking projects (IP, DNS, Routing, TCP/IP)
- Remote Support Tools & Techniques
- Certifications, including **Google IT Support Professional Certificate**

## 🛠️ Skills Featured
- Technical Support: Windows OS, remote assistance, documentation
- Networking: TCP/IP, DNS, DHCP, routing basics
- Tools: VirtualBox, Git/GitHub, Node.js, npm
- Soft Skills: Communication, problem-solving, teamwork

## 🔗 Other Projects
- Basic Troubleshooting Report [https://github.com/Nokuthula-ops/Basic-Troubleshooting-Report]
- Self-Healing PC Assistant [https://github.com/Nokuthula-ops/Self-Healing-PC-Assistant-project]
---

# Nokuthula Ngcongo 2025



